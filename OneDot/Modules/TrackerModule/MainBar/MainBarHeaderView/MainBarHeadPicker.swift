...
class MainBarHeadPicker: UIView, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        if currentLocation == "room" {
            for index in 0..<exercises.get(.room).count {
                if row == index {
                    UserDefaultsManager.shared.inRoomSave(row)
        ...
        if currentLocation == "street" {
                for index in 0..<exercises.get(.street).count {
                    if row == index {
                        UserDefaultsManager.shared.onTheStreetSave(row)
